...
# Function to fit the model and extract coefficients in OR, standard errors, and intervals
fit_model <- function(dependent, predictor) {
  ...
  # Extract results and calculate OR
  results <- model_fit %>%
    ...
    mutate(dependent_var = dependent, predictor = predictor)  # Add variable columns
  return(results)
}
